const { Plugin, TFile, Notice, requestUrl, PluginSettingTab, Setting } = require('obsidian');
    ...
    async fetchResults(count) {
        ...
        try {
            const response = await requestUrl({
                url: 'https://api.fabric.so/v2/search',
                method: 'POST',
                headers: {
            ...
            if (response.status >= 200 && response.status < 300) {
                const jsonData = await response.json; // Changed to use response.json()
...
class FabricLink extends Plugin {
    ...
    async insertTopResults(editor, cursor, count) {
        if (isNaN(count) || count <= 0) {
            count = 4; // Default to 4 if count is not a valid integer
        }
